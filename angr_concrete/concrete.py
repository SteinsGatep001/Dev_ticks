...
class CustomConcrete(SimStatePlugin):
    ...
    def copy(self, _memo):
        conc = CustomConcrete(segment_registers_initialized=self.segment_registers_initialized,
    ...
    def _sync_cle(self, target):
        ...
        def _check_mapping_name(cle_mapping_name, concrete_mapping_name):
            ...
            cle_mapping_name_match = re.findall(r"[\w']+", cle_mapping_name)
            concrete_mapping_name_match = re.findall(r"[\w']+", concrete_mapping_name)
            if cle_mapping_name_match[0] == concrete_mapping_name_match[0]:
                return True
            from pathlib import Path
            if Path(concrete_mapping_name).name == cle_mapping_name:
                return True
            return False
...
import angr.sim_options as options
